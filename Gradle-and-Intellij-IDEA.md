...
## Running Your Project
  ...
  * **iOS**: Before you can run on iOS you need to install RoboVM. See roboVM setup page (http://docs.robovm.com/getting-started/intellij.html) for more information on how to setup RoboVM. note: The free version is sufficient to compile, you just get less debug info. Once you have RoboVM, navigate to Run > Edit Configurations, and clicking the '+' to add a RoboVM iOS config, will open the dialog shown below.You can create multiple configurations, one for each device and simulator you want to debug on. After closing the dialog, select the Run menu and choose the configuration you just created.
