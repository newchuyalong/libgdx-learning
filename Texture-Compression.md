...
OpenGL ES 2.0 has only one mandatory texture compression format : ETC1. It allows to decrease the size of any RGB8 image by a 6 factor. The main drawback is that it is a lossy compression format. The other is that it is limited to RGB8: to support alpha channel, you have to to store alpha separately :
